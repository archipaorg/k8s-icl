...
::orch "kubernetes" "PodSecurityContext" as PodSecurityContext @runAsNonRoot, @fsGroup, @seLinuxOptions, @runAsUser, @supplementalGroups {
     /**Indicates that the container must run as a non-root user. If true, the Kubelet will validate
      ...
      be set in SecurityContext.  If set in both SecurityContext and PodSecurityContext, the
     ...
     1. The
      owning GID will be the FSGroup 2. The setgid bit is set (new files created in the volume
     ...
     /**The SELinux context to be applied to all containers. If unspecified, the container runtime
      will allocate a random SELinux context for each container.  May also be set in SecurityContext.

...
::orch "kubernetes" "PodSpec" as PodSpec @dnsPolicy, @serviceAccountName, @schedulerName, @hostNetwork, @restartPolicy, @hostIPC, @automountServiceAccountToken, @priorityClassName, @activeDeadlineSeconds, @securityContext, @nodeName, @hostAliases, @hostname, @serviceAccount, @nodeSelector, @priority, @terminationGracePeriodSeconds, @affinity, @containers, @volumes, @tolerations, @subdomain, @initContainers, @imagePullSecrets, @hostPID {
     ...
     /**If specified, indicates the pod's priority. "SYSTEM" is a special keyword which indicates
      ...
      with that name. If not specified, the pod priority will be default or zero if there is
      no default.*/

...
::orch "kubernetes" "JobSpec" as JobSpec @activeDeadlineSeconds, @backoffLimit, @completions, @manualSelector, @parallelism, @template, @selector {
     ...
     /**Specifies the desired number of successfully finished pods the job should be run with. 
      Setting to nil means that the success of any pod signals the success of all pods, and allows
      parallelism to have any positive value.  Setting to 1 means that parallelism is limited
      to 1 and the success of that pod signals the success of the job. More info: https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/*/
     completions = null,
     /**manualSelector controls generation of pod labels and pod selectors. Leave `manualSelector`
      ...
      in jobs that were created with the old `extensions/v1beta1` API. More info: https://git.k8s.io/community/contributors/design-proposals/selector-generation.md*/
     manualSelector = null,
     /**Specifies the maximum desired number of pods the job should run at any given time. The actual
      ...
      - .status.successful) < .spec.parallelism), i.e. when the work left to do is less than
      max parallelism. More info: https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/*/
     parallelism = null,

...
/***/
::orch "kubernetes" "NetworkPolicySpec" as NetworkPolicySpec @policyTypes, @egress, @ingress, @podSelector {
     /**List of rule types that the NetworkPolicy relates to. Valid options are Ingress, Egress,
      ...
      egress is allowed, you must specify a policyTypes value that include "Egress" (since such
      a policy would not include an Egress section and would otherwise default to just [ "Ingress"
     ...
     /**List of egress rules to be applied to the selected pods. Outgoing traffic is allowed if
      ...
      limits all outgoing traffic (and serves solely to ensure that the pods it selects are isolated

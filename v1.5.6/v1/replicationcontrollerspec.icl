...
::orch "kubernetes" "ReplicationControllerSpec" as ReplicationControllerSpec @replicas, @minReadySeconds, @template, @selector {
     ...
     /**Selector is a label query over pods that should match the Replicas count. If Selector is
      ...
      that must match in order to be controlled by this replication controller, if empty defaulted
      to labels on Pod template. More info: http://kubernetes.io/docs/user-guide/labels#label-selectors*/
     selector = null,
     /**Template is the object that describes the pod that will be created if insufficient replicas
      are detected. This takes precedence over a TemplateRef. More info: http://kubernetes.io/docs/user-guide/replication-controller#pod-template*/

...
::orch "kubernetes" "PodSpec" as PodSpec @dnsPolicy, @serviceAccountName, @hostNetwork, @restartPolicy, @hostIPC, @securityContext, @nodeName, @hostname, @serviceAccount, @nodeSelector, @activeDeadlineSeconds, @terminationGracePeriodSeconds, @hostPID, @volumes, @subdomain, @imagePullSecrets, @containers {
     /**Set DNS policy for containers within the pod. One of 'ClusterFirst' or 'Default'. Defaults
      to "ClusterFirst".*/
     dnsPolicy = null,
     ...
     /**Restart policy for all containers within the pod. One of Always, OnFailure, Never. Default
      to Always. More info: http://kubernetes.io/docs/user-guide/pod-states#restartpolicy*/
     restartPolicy = null,
     ...
     /**NodeSelector is a selector which must be true for the pod to fit on a node. Selector which
      must match a node's labels for the pod to be scheduled on that node. More info: http://kubernetes.io/docs/user-guide/node-selection/README*/
     nodeSelector = null,
     ...
     /**ImagePullSecrets is an optional list of references to secrets in the same namespace to use
      ...
      docker, only DockerConfig type secrets are honored. More info: http://kubernetes.io/docs/user-guide/images#specifying-imagepullsecrets-on-a-pod*/
     imagePullSecrets = null,
     /**List of containers belonging to the pod. Containers cannot currently be added or removed.
      There must be at least one container in a Pod. Cannot be updated. More info: http://kubernetes.io/docs/user-guide/containers*/
     containers = null,
